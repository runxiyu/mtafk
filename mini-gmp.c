...
static void *gmp_default_realloc(void *old, size_t unused_old_size,
				 size_t new_size)
{
	void *p;
...
{
	assert(size > 0);
	return (mp_ptr) (*gmp_reallocate_func) (old, 0,
		...
		mp_limb_t ux)
{
	...
	while (limb == 0) {
		...
		if (i == un)
			return (ux ==
				0 ? ~(mp_bitcnt_t) 0 : un * GMP_LIMB_BITS);
		limb = ux ^ up[i];
	}
	gmp_ctz(cnt, limb);
...
mp_bitcnt_t mpn_scan0(mp_srcptr ptr, mp_bitcnt_t bit)
{
	...
	return mpn_common_scan(~ptr[i] &
			       (GMP_LIMB_MAX << (bit % GMP_LIMB_BITS)), i, ptr,
			       i, GMP_LIMB_MAX);
}
...
static void
mpn_div_qr_2_invert(struct gmp_div_inverse *inv, mp_limb_t d1, mp_limb_t d0)
{
	unsigned shift;
...
static void
mpn_div_qr_invert(struct gmp_div_inverse *inv, mp_srcptr dp, mp_size_t dn)
{
	assert(dn > 0);
	       ...
	       mp_srcptr dp, mp_size_t dn, mp_limb_t dinv)
{
	...
	do {
		...
		if (n1 == d1 && n0 == d0) {
			...
			gmp_udiv_qr_3by2(q, n1, n0, n1, n0, np[dn - 2 + i], d1,
					 d0, dinv);
			...
			if (cy != 0) {
				n1 +=
				    d1 + mpn_add_n(np + i, np + i, dp, dn - 1);
				q--;
			}
		}
...
mpn_div_qr_preinv(mp_ptr qp, mp_ptr np, mp_size_t nn,
		  mp_srcptr dp, mp_size_t dn, const struct gmp_div_inverse *inv)
{
	assert(dn > 0);
	assert(nn >= dn);
		 ...
		 const struct gmp_div_inverse *binv)
{
	...
	for (i = 0; w > 0; i++) {
		...
		assert((r & (GMP_LIMB_MAX >> (GMP_LIMB_BITS - binv->shift))) ==
		       0);
		r >>= binv->shift;
...
void mpz_set_si(mpz_t r, signed long int x)
{
	if (x >= 0)
		mpz_set_ui(r, x);
	else /* (x < 0) */ if (GMP_LIMB_BITS < GMP_ULONG_BITS) {
	...
	if (x > 0) {
		...
		if (GMP_LIMB_BITS < GMP_ULONG_BITS) {
			...
			while (x >>= LOCAL_GMP_LIMB_BITS) {
				++r->_mp_size;
				MPZ_REALLOC(r, r->_mp_size)[r->_mp_size - 1] =
				    x;
			}
		}
	} else
...
static int mpn_absfits_ulong_p(mp_srcptr up, mp_size_t un)
{
	...
	if (GMP_ULONG_BITS % GMP_LIMB_BITS != 0)
		ulongrem =
		    (mp_limb_t) (ULONG_MAX >> GMP_LIMB_BITS * ulongsize) + 1;

	return un <= ulongsize || (up[ulongsize] < ulongrem
				   && un == ulongsize + 1);
}
...
void mpz_mod(mpz_t r, const mpz_t n, const mpz_t d)
{
	mpz_div_qr(NULL, r, n, d,
		   d->_mp_size >= 0 ? GMP_DIV_FLOOR : GMP_DIV_CEIL);
}
...
void mpz_powm(mpz_t r, const mpz_t b, const mpz_t e, const mpz_t m)
{
	...
	if (e->_mp_size < 0) {
		...
		bn = base->_mp_size;
		if (bn >= mn) {
			mpn_div_qr_preinv(NULL, base->_mp_d, base->_mp_size, mp,
					  mn, &minv);
			bn = mn;
		}
	...
	while (--en >= 0) {
		...
		do {
			...
			if (w & bit)
				mpz_mul(tr, tr, base);
			if (tr->_mp_size > mn) {
				mpn_div_qr_preinv(NULL, tr->_mp_d, tr->_mp_size,
						  mp, mn, &minv);
				tr->_mp_size =
				    mpn_normalized_size(tr->_mp_d, mn);
			}
			bit >>= 1;
		}
		while (bit > 0);
...
static int
gmp_lucas_mod(mpz_t V, mpz_t Qk, long Q, mp_bitcnt_t b0, const mpz_t n)
{
	mp_bitcnt_t bs;
	mpz_t U;
	int res;
...
static void mpz_abs_add_bit(mpz_t d, mp_bitcnt_t bit_index)
{
	...
	if (limb_index >= dn) {
		...
		cy = mpn_add_1(dp + limb_index, dp + limb_index,
			       dn - limb_index, bit);
		if (cy > 0) {
...
char *mpz_get_str(char *sp, int base, const mpz_t u)
{
	...
	if (bits)
		...
		sn = i + mpn_get_str_bits((unsigned char *)sp + i, bits,
					  u->_mp_d, un);
	else {
		struct mpn_base_info info;
		mp_ptr tp;
		...
		sn = i + mpn_get_str_other((unsigned char *)sp + i, base, &info,
					   tp, un);
		gmp_free(tp);
	}
